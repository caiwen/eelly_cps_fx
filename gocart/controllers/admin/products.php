...
class Products extends Admin_Controller {	
	...
	function sync_eelly_goods()
	{
	    ...
	    $getObject = $this->rest->get('api/mall_items/detail_get.json',array('goods_id'=>$id,'user_id'=>$current_admin['webmaster_id']));
	    $getArray=object_to_array($getObject);
	    if($getArray['mall_items.detail_get']['success']) {
	    	$data=$this->mappedField($getArray['mall_items.detail_get']['data'],$id);
	    	echo $this->add_one_good($data);
	    }else {
	    	echo json_encode(array('success'=>false,'msg'=>'同步失败','data'>''));
	    }
	}
	
	function add_one_good($data)
	{
	    $save['id']					= $data['id'];
	    $save['sku']				= $data['sku'];
	    $save['name']				= $data['name'];
	    $save['seo_title']			= $data['seo_title'];
	    $save['meta']				= $data['meta'];
	    $save['description']		= $data['description'];
	    $save['click_url']          = $data['click_url'];
	    $save['excerpt']			= $data['excerpt'];
	    $save['price']				= $data['price'];
	    $save['saleprice']			= $data['saleprice'];
	    $save['quantity']			= $data['quantity'];
	    $save['eelly_thumb_img']    = $data['eelly_thumb_img'];
	    $save['track_stock']        =1;
	    //$post_images				= $this->input->post('images');
	    ...
	    // save product
	    $product_id	= $this->Product_model->save($save, $options, $categories,false);
	...
	function mappedField($data,$goods_id)
	{
	    $this->load->helper('pinyin');
	    $returnArr=array();
        $returnArr['name']=$data['title'];
		$returnArr['sku']=$data['sn'];
		$returnArr['seo_title']=$data['title'].'_'.$this->config->item('company_name');
		$returnArr['meta']='<meta name="request" content="-app-goods-id-"'.$goods_id.' /><meta http-equiv="Content-Type" content="text/html; charset=utf-8" /><meta http-equiv="X-UA-Compatible" content="IE=EmulateIE7 charset=utf-8" /><meta name="description" content="'.$data['title'].'" /><meta name="keywords" content="'.$data['title'].'" />';
		$returnArr['description']=$data['description'];
		$returnArr['click_url']=$data['url'];
		$returnArr['excerpt']=$data['title'];
		$returnArr['price']=$data['maxgoodsprice'];
		$returnArr['saleprice']=$data['price'];
		$returnArr['quantity']=$data['stock'];
		$returnArr['id']=$goods_id;
		$returnArr['categories']=array($data['cate1'],$data['cate2']);
		$returnArr['slug']='shangpin'.$goods_id;
		$returnArr['eelly_thumb_img']=$data['image'];
		return $returnArr;
	}
