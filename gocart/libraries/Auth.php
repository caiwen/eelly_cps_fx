	...
	*/
	function login_admin($email, $password, $remember=false)
	{
		...
		if (sizeof($result) > 0)
		{
			...
			//find webmaster info
			$webmasterinfo=$this->CI->db->select('*')
			->where('admin_id',$result['id'])
			->limit(1)->get('webmaster_info')->row_array();
			if(!empty($webmasterinfo)) {
			  $admin['admin']['webmaster_id']	= $webmasterinfo['webmaster_id'];
			  $admin['admin']['webmaster_mail']	= $webmasterinfo['webmaster_mail'];
			}
